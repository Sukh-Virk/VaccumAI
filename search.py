...
class VacuumPlanning(Problem):
    ...
    def path_cost(self, curNode, state1, action, state2):
        ...
        if self.env.costFunc == 'Step':  # Basic step count
            cost += 1

        elif self.env.costFunc == 'StepTurn':  # Step and turn cost
            turn_cost = self.computeTurnCost(curNode.action, action)
            cost += 1 + turn_cost

        elif self.env.costFunc == 'StayLeft':  # Penalize moving right
            cost += state2[0]

        elif self.env.costFunc == 'StayUp':  # Encourage staying up
            cost += 1 + (state2[1] / len(self.map))

            # Apply additional penalty for moving down when using UCS with StayUp
            if self.env.searchType == 'UCS' and action == 'DOWN':
                cost += 4 # Add a penalty for the DOWN action in UCS StayUp mode

        return cost

    def computeTurnCost(self, action1, action):
        """Computes turn cost as the number of 90-degree rotations."""
        directions = ['UP', 'RIGHT', 'DOWN', 'LEFT']
        if action1 not in directions or action not in directions:
            return 0  # No cost if actions are not directions
        ...
        return min(turn_cost, 4 - turn_cost) * 3  # Each 90-degree turn costs 3
    ...
    def best_first_graph_search(problem, f=None):
        """
        Perform a best-first graph search with dynamic pruning to minimize exploration.

        Parameters:
            problem: An instance of the Problem class.
            f: A function that evaluates nodes, such as the sum of path_cost and heuristic for A*.

        Returns:
            A tuple containing the solution node and a list of explored states.
        """
    ...
    def findMinManhattanDist(self, pos):
        """
        Finds the dirty room with the minimum Manhattan distance to the given position.

        Parameters:
            pos (tuple): The current position of the agent as (x, y).

        Returns:
            int: The minimum Manhattan distance to the nearest dirty room.
        """
        # Get all dirty room locations
        dirty_rooms = [thing.location for thing in self.env.things if isinstance(thing, Dirt)]

        if not dirty_rooms:
            return 0  # If no dirty rooms, return zero

        # Calculate the Manhattan distance to each dirty room
        return min(abs(pos[0] - room[0]) + abs(pos[1] - room[1]) for room in dirty_rooms)

    def findMinEuclidDist(self, pos):
        """
        Finds the dirty room with the minimum squared distance to the given position.

        Parameters:
            pos (tuple): The current position of the agent as (x, y).

        Returns:
            float: The minimum squared distance to the nearest dirty room.
        """
        # Get all dirty room locations
        dirty_rooms = [thing.location for thing in self.env.things if isinstance(thing, Dirt)]
        ...
        # Calculate the squared distance to each dirty room
...
def breadth_first_graph_search(problem):
    ...
    node = Node(problem.initial)
    if problem.goal_test(node.state):
        return node, [tuple(node.state)]  # Return the Node and explored states as a list

    frontier = deque([node])  # FIFO queue
    explored = set()

    while frontier:
        node = frontier.popleft()  # Get the shallowest node
        ...
        for action in problem.actions(node.state):
            ...
            if tuple(child.state) not in explored and child not in frontier:
                if problem.goal_test(child.state):
                    return child, list(explored)  # Return the Node and explored states

                frontier.append(child)

    return None, list(explored)  # If no solution is found, return None and explored states
...
def depth_first_graph_search(problem):
    ...
    while frontier:
        node = frontier.pop()  # Get the deepest node
        explored.add(tuple(node.state))  # Ensure states are hashable (e.g., tuples)
